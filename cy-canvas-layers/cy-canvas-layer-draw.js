...
export default class CyCanvasLayerDraw extends CyCanvasLayer {
    ...
    setStyle(layerId, newLayer) {
        ...
        ((ly.name = newLayer.name), (ly.layerStyle = newLayer.layerStyle));
        return old;
    }
     ...
     * @returns
     */
    addBlocks(layerId = this._activeLayerId, paths) {
        //pueden ser circles o polygons también...
        if (paths === undefined) {
            console.log("tipo de bloque no visualizable");
            return;
        }
        ...
        ps.forEach((b) => {
            if (b.type !== "point" && b.type !== "cut-point") {
                b.data = {
                    id: `B${layer.nextBlockId++}`,
                    lyId: layerId,
                    canvasPath: getPathFromBlocks(b),
                    points: [],
                };
    ...
    setActiveLayerId(lyId) {
        if (lyId) this._activeLayerId = lyId;
    ...
    getLayer(layerId) {
        return this.layers.find((ly) => ly.lyId === layerId);
    }
    getActiveLayer() {
        return this.layers.find((ly) => ly.lyId === this._activeLayerId);
    }
    layerIsEmpty(layerId) {
        return this.getLayer(layerId).blocks.length === 0;
    }
    /**@todo Los cuts los hago intercapas, así que no llevan layerId */
    //Y por otra parte pertenecen a 2 bloques, ... igual son eternos

    addCutPoints(points) {
        //estos ya vienen pretratados
        if (!points) {
    ...
    getNearestPoint(x, y, w) {
        ...
        if (ps.length > 0) {
            //y si estamos mirando puntos, sería
            ...
            for (let jx = 0; jx < blocks.length; jx++) {
                ...
                if (this.ctx.isPointInStroke(b.data.canvasPath, p.x, p.y)) {
                    if (b.type !== "path") {
                        ...
                        //path, miro dentro, que no tiene el path generado,
    ...
    hover(x, y, box, select = false) {
        let blocks;
        if (this.selectData.hoveredBlocks) this.selectData.hoveredBlocks.forEach((b) => (b.hover = false)); //gestión de hover, aquí borra anarillo
        if (box) {
