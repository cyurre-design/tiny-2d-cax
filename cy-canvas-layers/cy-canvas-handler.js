...
export function scaleMm2pixels(x) {
    return +x * scale;
}
//los offsets en canvas son 0, siempre va de (0,0) a (width,height) pero dado la vuelta el y
export function position2pixels(p) {
    return { x: (p.x - extents.x) * scale, y: dim.height - scale * (p.y - extents.y) };
}
export function scalePixels2mm(x) {
    return +x / scale;
}
//Supongo unidades canvas, no client, eso se lo resto directamente en el tratamiento del evento, si lo hay
export function pixels2position(p) {
    return { x: extents.x + p.x / scale, y: extents.y + (dim.height - p.y) / scale };
} //signo y !!!
...
export default class CyCanvasHandler {
    constructor(canvas, yAxisDownwards = true) {
        ...
        this.application = {
            mouseMove: (pi, evt) => {},
            leftClick: (pi, evt) => {
                if (this.z) this.canvas.parentNode.removeChild(this.z);
            },
            leftDblclick: (pi, evt) => {},
            ...
            leftClickMove: (pi, pf) => {
                if (this.z) {
                    ...
                    if (w < 10 || h < 10) return;
                    let p = {
                        x: this.initialPoint.x < this.finalPoint.x ? this.initialPoint.x : this.finalPoint.x,
    ...
    mouseup(evt) {
        ...
        this.pf = pixels2position(this.finalPoint);
        let isShortStroke =
            Math.abs(this.finalPoint.x - this.initialPoint.x) < this.mouseTolerance &&
            Math.abs(this.finalPoint.y - this.initialPoint.y) < this.mouseTolerance;
        if (evt.which === 1) {
            //left button pressed
    ...
    view(cmd, m, e) {
        switch (
            cmd //teclas, el 0.9 es obviamente opcional
        ) {
