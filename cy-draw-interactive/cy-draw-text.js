...
export class DrawText extends DrawBasic {
	...
	updateData = (data) => {
		...
		switch (idn) {
			case "a":
				this.data[idn] = (+value * Math.PI) / 180;
				break;
			case "radius":
			case "size":
				this.data[idn] = +value;
			...
			case "way":
				...
				break;
			case "invert":
				this.data.invert = value === "l2r" ? false : true;
				break;
			case "font":
				{
					const buffer = fetch(`/useFont?fontName=${value}`).then((res) => res.arrayBuffer());
					buffer.then((data) => {
						//Hay que esperar la promesa para dibujar
				...
				break;
			case "insert": {
				//genero un evento por path...
				this.clear();
				this.basicGeometry.forEach((path) =>
					this.layerDraw.dispatchEvent(new CustomEvent("new-block", { bubbles: true, composed: true, detail: { type: "path", data: path } })),
				);
