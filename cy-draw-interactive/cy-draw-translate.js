...
export class DrawTranslate extends DrawBasic {
	constructor(layerDraw, subMode) {
		...
		this.clickFn = [
			[this.p0, this.getBlocks],
			[this.m1, this.translate, this.deleteData],
		];
		this.dataSent = [
			["x0", "y0"],
			["x1", "y1"],
	...
	move = (pi) => {
		...
		this.hit = this.highLight(
			pi.x,
			pi.y,
			this.blocksToMove.map((b) => blockTranslate(b, pi.x - this.data.x0, pi.y - this.data.y0)),
		);
	};
	translate = (/*p*/) => {
		this.layerDraw.dispatchEvent(
			new CustomEvent("geometry-transform", {
				bubbles: true,
				composed: true,
				detail: { command: "translate", data: { dx: this.hit.x - this.data.x0, dy: this.hit.y - this.data.y0 } },
			}),
		);
		this.status = 0;
	};
	...
	updateData = (data) => {
		...
		switch (idn) {
			case "enter":
				this.translate();
				this.status = 0;
				this.move({ x: this.data.x1, y: this.data.y1 });
				break;
			case "esc":
				this.status = 0;
				break;
			...
			case "x0":
			case "y0":
				this.p0({ x: this.data.x0, y: this.data.y0 });
				break;
			case "x1":
			case "y1":
				this.move({ x: this.data.x1, y: this.data.y1 });
