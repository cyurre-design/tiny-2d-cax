...
export class DrawCircle extends DrawBasic {
			...
			case "3P":
				{
					this.moveFn = [
						...
						[this.h, this.m2, this.sendDataBasic, this.draw],
					];
	...
	m2 = (pi) => {
		[this.data.x2, this.data.y2] = this.hit !== undefined ? [this.hit.x, this.hit.y] : [pi.x, pi.y];
	};
	draw = (pi) => {
		this.hit = this.highLight(pi.x, pi.y, createDrawElement("circle", this.data));
	};
}
