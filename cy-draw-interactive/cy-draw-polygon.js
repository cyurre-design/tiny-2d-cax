...
export class DrawPolygon extends DrawBasic {
	...
	newBlock = (/*p*/) => {
		this.layerDraw.dispatchEvent(new CustomEvent("new-block", { bubbles: true, composed: true, detail: { type: "polygon", data: this.data } }));
	};
	draw = (pi) => {
		this.hit = this.highLight(pi.x, pi.y, createDrawElement("polygon", this.data));
	};
	deleteData = () => {
		this.deleteDataBasic(["x0", "x1", "y0", "y1", "cx", "cy"]);
