...
export class DrawSegmentBB extends DrawBasic {
	constructor(layerDraw, submode) {
		...
		this.clickFn = [
			[this.m0, this.save],
			[this.m1, this.tangentb, this.newBlock, this.deleteData, this.clear],
		];
		this.dataSent = [[], [], []];
		this.dataReceived = [];
	}
	...
	save = (/*pi*/) => {
		if (this.block) {
			//si no hay pinchado no avanzamos
	...
	tangentb = (/*pi*/) => {
		...
		sol = sol.sort(
			(s1, s2) =>
				distancePointToPoint(s1[0].x, s1[0].y, this.data.x0, this.data.y0) - distancePointToPoint(s2[0].x, s2[0].y, this.data.x0, this.data.y0),
		);
		sol = sol.slice(0, 2); //Las dos soluciones serán una exterior y otra interior, previsiblemente
		sol = sol.sort(
			(s1, s2) =>
				distancePointToPoint(s1[1].x, s1[1].y, this.data.x1, this.data.y1) - distancePointToPoint(s2[1].x, s2[1].y, this.data.x1, this.data.y1),
		);
	...
	draw = (pi) => {
		this.hit = this.highLight(pi.x, pi.y, [createDrawElement("segment", this.data)]);
	};
}
