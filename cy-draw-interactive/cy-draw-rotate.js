...
export class DrawRotate extends DrawBasic {
	...
	move = (pi) => {
		...
		this.hit = this.highLight(
			pi.x,
			pi.y,
			this.blocksToRotate.map((b) => blockRotate(b, this.data.x0, this.data.y0, (this.data.a * Math.PI) / 180)),
		);
	};
	rotate = (/*p*/) => {
		this.layerDraw.dispatchEvent(
			new CustomEvent("geometry-transform", {
				bubbles: true,
				composed: true,
				detail: { command: "rotate", data: { x: this.data.x0, y: this.data.y0, a: (this.data.a * Math.PI) / 180 } },
			}),
		);
		this.status = 0;
	};
	...
	updateData = (data) => {
		...
		switch (idn) {
			...
			case "a":
				if (this.status !== 0) this.move({ x: this.data.x0, y: this.data.y0 });
				break;
		}
	};
}
