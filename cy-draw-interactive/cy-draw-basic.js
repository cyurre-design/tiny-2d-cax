...
export default class DrawBasic {
	...
	getBlocks = () => {
		this.blocksToMove = this.layerDraw.getSelectedBlocks();
	};
	...
	m1 = (pi) => {
		[this.data.x1, this.data.y1] = this.hit !== undefined ? [this.hit.x, this.hit.y] : [pi.x, pi.y];
	};

	h = (pi) => {
		this.hit = this.highLight(pi.x, pi.y, undefined);
	};

	sendDataBasic = () => {
		this.layerDraw.dispatchEvent(
			new CustomEvent("drawing-event", { bubbles: true, composed: true, detail: { data: this.dataBasic(this.dataSent[this.status]) } }),
		);
	};
	dataBasic(keys) {
	...
	leftClick = (pi /*, evt*/) => {
		let p = this.hit || pi;
		this.clickFn[this.status].forEach((f) => {
			if (this.enabled) f(p);
	...
	updateDataBasic = (data) => {
		...
		Object.keys(data).forEach((k) => {
			...
			if (this.dataReceived.includes(k)) this.data[k] = +data[k];
		});
		return newData;
	};
}
