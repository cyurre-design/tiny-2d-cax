...
function round(x)   { return (linkTolerance*Math.round( linkToleranceInv * x))}
...
function _entities2Geometry(entities, blocks){
    ...
    capas.forEach(layer => {
        ...
        let blocks = [], paths = [], circles=[];
        //let path = createDrawElement('path', {elements: []}); //dxf cambiará...es por poner algo
        ents.forEach((e, ix)=>{
            switch (e.type) {
                case 'LINE':
                {
                    blocks.push(createDrawElement('segment', {subType:"PP", x0: e.vertices[0].x, y0: e.vertices[0].y, x1: e.vertices[1].x, y1: e.vertices[1].y}));
                ...
                case 'ARC':{
                    ...
                    blocks.push(createDrawElement('arc', {subType:"DXF", cx: e.center.x, cy: e.center.y, r: e.radius, ai: e.startAngle, af: e.endAngle}));
                    // const pix = e.vertices[0].x, piy = e.vertices[0].y;
                    // const pfx = e.vertices[1].x, pfy = e.vertices[1].y;
