...
function path2geo(commands) {
    ...
    commands.forEach((cmd) => {
        if (!fuzzy_eq(cmd.x, pos.x) || !fuzzy_eq(cmd.y, pos.y)) {
            //porque si no genera a veces segmentos nulos
            switch (cmd.type) {
                case "M":
                ...
                case "Q": //quadratic Bezier, pero la rutina de cúbica debe adaptarlo
                    elements.push(
                        createDrawElement("bezier", { subType: "Q", x0: pos.x, y0: pos.y, cp1x: cmd.x1, cp1y: cmd.y1, x1: cmd.x, y1: cmd.y }),
                    );
                    break;
                default:
                    console.log("ERROR");
...
export function textToGeometry(text, font, size) {
    ...
    geoPaths = geoPaths.map((p) => {
        //Hago COPIA de los bbs porque luego voy a modificarlos y por referencia se machacan
        //        let b = bboxes[p.ix];
...
export function transformTextGeometry(paths, radius, alfa, way, invert) {
    ...
    if (textWidth >= 2 * Math.PI * r)
        //por si meten más caracteres que los que caben, aumentamos el tamaño
    ...
    if (way === "antiClock") {
        ...
        geoPaths = geoPaths.map((p) => {
            //punto de referencia será cx, r +h
