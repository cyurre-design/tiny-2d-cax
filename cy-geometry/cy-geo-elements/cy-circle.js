"use strict";
import { pointSymmetricSegment, rotateZ, scale0 } from "../cy-geometry-library.js";
...
export function createCircle(data = {}) {
    const circle = {
        ...
        cy: data.cy,
        x0: data.cx,
        y0: data.cy, //el alias mejora mucho la unificación de nombres en algunos casos
        x1: data.x1,
        y1: data.y1,
        r: Math.hypot(data.x1 - data.cx, data.y1 - data.cy),
        type: "circle",
        //get p0(){ return ({x:this.x0,y:this.y0})}, get pi(){ return ({x:this.x1,y:this.y1})} , get pf(){ return ({x:this.x1,y:this.y1})}
    };
    circle.bbox = circleBbox(circle);
    return circle;
}
export function circleBbox(circle) {
    return { x0: circle.cx - circle.r, y0: circle.cy - circle.r, x1: circle.cx + circle.r, y1: circle.cy + circle.r };
}
export function circleClone(circle) {
    return JSON.parse(JSON.stringify(circle));
}
