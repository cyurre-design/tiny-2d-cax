...
import {
    ...
    circleSymmetryL,
    circleLength,
} from './cy-geo-elements/cy-circle.js'
import {
    polygonTranslate,
    polygonRotate,
    polygonScale,
    ...
    polygonSymmetryL,
    polygonLength,
} from './cy-geo-elements/cy-polygon.js'
import {
    pathTranslate,
    pathRotate,
    pathScale,
...
import {
    ...
    bezierSymmetryL,
    bezierReverse,
} from './cy-geo-elements/cy-bezier.js'
...
export function segmentTangentToArc(c, x, y) {
    ...
    if (Math.abs(c.cx - x) >= Math.abs(c.cy - y)) {
        //x=my+x0
        let m = -(c.cy - y) / (c.cx - x)
        let x0 = -rest / (c.cx - x)
...
export function segmentsTangentToCircleAndCircle(c0, c1) {
    ...
    if (dcc >= dr) {
        //Hay tangentes exteriores por lo menos, aunque igual solo una...
        //Tangentes exteriores
        if (fuzzy_eq_zero(dr)) {
            //circulos de mismo radio, las tangentes no se cortan sino que son paralelas
            ...
            //xp,yp es el punto de corte de las tangentes exteriores, que estará a la izquierda o a la derecha de ambos
    ...
    if (dcc >= c0.r + c1.r) {
        //Hay tangentes interiores que se cortan en xp,yp
        xp = (c1.cx * c0.r + c0.cx * c1.r) / (c0.r + c1.r)
        yp = (c1.cy * c0.r + c0.cy * c1.r) / (c0.r + c1.r)
        //con el punto puedo obtener el seno  (vale cualquiera de los circulos)
        const s = c0.r / Math.hypot(c0.cx - xp, c0.cy - yp)
...
export function circleFrom3Points(p1, p2, p3) {
    ...
    if (denominador === 0) {
        ...
        data.x1 = p1.x
        data.y1 = p1.y //por definir un punto inicial
        return data
    }
}
...
export function arc2PC2SVG(c, r, pi, pf, way) {
    //data va por referencia y se le "pegan" los flags
...
export function arcCPA(c, pi, a) {
    ...
    const cos = Math.cos(data.da),
        sin = Math.sin(data.da)
...
export function arcDxf(data) {
    ...
    const x1 = cx + r * Math.cos(ai),
        y1 = cy + r * Math.sin(ai)
    const x2 = cx + r * Math.cos(af),
        y2 = cy + r * Math.sin(af)
...
export function blockTranslate(block, dx, dy) {
    switch (block.type) {
        case 'segment':
            return segmentTranslate(block, dx, dy)
        case 'circle':
        ...
        case 'path':
            return pathTranslate(block, dx, dy)
        case 'bezier':
            return bezierTranslate(block, dx, dy)
        case 'point':
            return translatePoint(block.x0, block.y0, dx, dy)
        default:
...
export function blockRotate(block, x, y, alfa) {
    switch (block.type) {
        case 'segment':
            return segmentRotate(block, x, y, alfa)
        case 'circle':
        ...
        case 'path':
            return pathRotate(block, x, y, alfa)
        case 'bezier':
            return bezierRotate(block, x, y, alfa)
        default:
            console.log('no contemplado')
    }
}
export function blockScale(block, x, y, scale) {
    switch (block.type) {
        case 'segment':
            return segmentScale(block, x, y, scale)
        case 'circle':
        ...
        case 'path':
            return pathScale(block, x, y, scale)
        case 'bezier':
            return bezierScale(block, x, y, scale)
        default:
            console.log('no contemplado')
    }
}
export function blockSymmetryX(block, y) {
    switch (block.type) {
        case 'segment':
            return segmentSymmetryX(block, y)
        case 'circle':
        ...
        case 'path':
            return pathSymmetryX(block, y)
        case 'bezier':
            return bezierSymmetryX(block, y)
        default:
            console.log('no contemplado')
    }
}
export function blockSymmetryY(block, x) {
    switch (block.type) {
        case 'segment':
            return segmentSymmetryY(block, x)
        case 'circle':
        ...
        case 'path':
            return pathSymmetryY(block, x)
        case 'bezier':
            return bezierSymmetryY(block, x)
        default:
            console.log('no contemplado')
    }
}
export function blockSymmetryL(block, s) {
    switch (block.type) {
        case 'segment':
            return segmentSymmetryL(block, s)
        case 'circle':
        ...
        case 'path':
            return pathSymmetryL(block, s)
        case 'bezier':
            return bezierSymmetryL(block, s)
        default:
            console.log('no contemplado')
    }
}
export function blockReverse(block) {
    switch (block.type) {
        ...
        case 'polygon':
            return () => {}
        case 'path':
            return pathReverse(block)
        case 'bezier':
            return bezierReverse(block)
        default:
            console.log('no contemplado')
    }
}
...
export function blockLength(block) {
    switch (block.type) {
        ...
        case 'circle':
            return circleLength(block)
        case 'arc':
            return arcLength(block)
        case 'polygon':
            return polygonLength(block)
        case 'path':
            return pathLength(block)
...
export function blockMidpoint(block) {
    switch (block.type) {
        case 'segment':
            return segmentMidpoint(block)
        case 'circle':
            return { x: block.cx - block.r, y: block.cy }
        case 'arc':
            return arcMidpoint(block)
        case 'polygon':
...
export function blockSplitAtPoints(block, points) {
    switch (block.type) {
        case 'segment':
            return segmentSplitAtPoints(block, points)
        //case 'circle': return circleSplitAtPoints(block, points);
        case 'arc':
            return arcSplitAtPoints(block, points)
        case 'polygon':
        case 'path':
        case 'bezier':
        default:
