...
import { fuzzy_eq_point, fuzzy_eq_zero, geometryPrecision, distancePointToPoint, sqDistancePointToPoint } from "./cy-geometry-library.js";
...
export function line_line_intr(s1, s2, eps = geometryPrecision) {
    ...
    let u2 = {x:s2.pf.x-s2.pi.x, y:s2.pf.y-s2.pi.y};
    let u1 = {x:s1.pf.x-s1.pi.x, y:s1.pf.y-s1.pi.y};
    let w = {x:s1.pi.x-s2.pi.x, y:s1.pi.y-s2.pi.y};
    ...
    let wx = s2.ux*w.x + s2.uy*w.y;     // vector de s1 a s2
    ...
    if(fuzzy_eq_zero(u1x, eps) && fuzzy_eq_zero(u1y, eps)){ //s1.pf === s1.pi puesto de otra manera...
        if (fuzzy_eq_zero(u2x, eps)){    //ambos son un puntp
    ...
    if(fuzzy_eq_zero(u2x)){ //el s2 es punto pero el s1 no lo es. mirar si está dentro
    ...
    if(!fuzzy_eq_zero(denominador, eps)){    //NO SON COLINEALES, CASO NORMAL  
        ...
        if(( distancePointToPoint(p.x, p.y, s1midpoint.x, s1midpoint.y ) > 0.5*s1.d + eps)
    ...
    if(!fuzzy_eq_zero(distancePointToLine(s1.pi, s2), eps))
        return {r:Cut.NoIntersect};
    ...
    if(wx < u1x){ //orientado como s2, al menos 1 punto debe estar dentro
