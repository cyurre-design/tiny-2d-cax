    ...
    result = path.elements.map((shape, ix) => {
        ...
        if (shape.type === "segment") {
            ...
            if (newr < 0 || fuzzy_eq_zero(newr, geometryPrecision)) {
                ...
                //escalo (r+offset)/r con centro de escalada en el centro del arco
...
// originalmente existía la rutina de unir mediante arco, pero casi queda más legible la creación del arco explícita
 ...
 * @param {parámetros} join_params
 * @returns el bloque o bloques generados y lo que queda de mirar
 */
function line_line_join(s1, s2, path, join_params) {
    ...
    if (s1.collapsed_arc || s2.collapsed_arc) {
        ...
        let ns = createArc(
            arc2PC2SVG({ x: origin.pf.x, y: origin.pf.y }, Math.abs(join_params.offset), s1.pf, s2.pi, join_params.connection_arcs_ccw),
        );
        ...
        switch (res.r) {
            case Cut.NoIntersect: {
                //ejemplo? No termino de ver cómo se producen las paralelas...
                // parallel lines, join with half circle
                let ns = createArc(
                    arc2PC2SVG(
                        { x: 0.5 * (s1.pf.x + s2.pi.x), y: 0.5 * (s1.pf.y + s2.pi.y) },
                        0.5 * distancePointToPoint(s1.pf.x, s1.pf.y, s2.pi.x, s2.pi.y),
                        s1.pf,
                        s2.pi,
                        join_params.connection_arcs_ccw,
                    ),
                );
                return { res: [s1, ns], left: s2 };
            }
            case Cut.TrueIntersect: {
                //modifico el final de s1 y el comienzo de s2
                let point = res.point;
            ...
            case Cut.Overlapping: {
                //supongo que quiere colapsar un trozo de segmento, no parece muy habitual...
                //sería cuando al compensar dos líneas paralelas salen dos segmentos que coinciden
            ...
            case Cut.FalseIntersect: {
                //Por ejemplo dos segmentos en ángulo que al compensar por fuera obviamente no se cortan
                //Devuelvo arco siempre, queda más chulo. pero podría ser un segmento
                let origin = path.elements[s1.origin];
                let ns = createArc(
                    arc2PC2SVG({ x: origin.pf.x, y: origin.pf.y }, Math.abs(join_params.offset), s1.pf, s2.pi, join_params.connection_arcs_ccw),
                );
...
function line_arc_join(s, a, path, join_params) {
    ...
    if (res.r === Cut.NoIntersect) {
        ...
        if (res.r === Cut.TwoIntersects) {
            //un arco sí puede tener dos cortes con un segmento
        ...
        const ns2 = createArc(arc2PC2SVG({ x: a.cx, y: a.cy }, a.r, point, a.pf, a.way));
        ns1.origin = s.origin;
        ns2.origin = a.origin;
        return { res: [ns1], left: ns2 };
    }
...
function arc_line_join(a, s, path, join_params) {
    ...
    if (res.r === Cut.NoIntersect) {
        ...
        //'TangentIntersect','TwoIntersects','OneIntersect'
        let point;
        if (res.r === Cut.TwoIntersects) {
            let dist1 = sqDistancePointToPoint(res.point1.x, res.point1.y, a.pf.x, a.pf.y);
            let dist2 = sqDistancePointToPoint(res.point2.x, res.point2.y, a.pf.x, a.pf.y);
            point = dist1 < dist2 ? res.point1 : res.point2;
        } else {
            point = res.point;
        }
        const ns1 = createArc(arc2PC2SVG({ x: a.cx, y: a.cy }, a.r, a.pi, point, a.way));
...
function arc_arc_join(a1, a2, path, join_params) {
    ...
    if (res.r === Cut.OverlappingArcs) {
        //todo, buscar el caso malo que pase por aquí
        console.log("TODO"); //El original parece no hacer nada
        let ns1, ns2;
        if (res.sameDirection === true) {
            ns1 = createArc(arc2PC2SVG({ x: a1.cx, y: a1.cy }, a1.r, a1.pi, res.point1, a1.way));
            ns2 = createArc(arc2PC2SVG({ x: a2.cx, y: a2.cy }, a2.r, res.point2, a2.pf, a2.way));
        } else {
            ns1 = createArc(arc2PC2SVG({ x: a1.cx, y: a1.cy }, a1.r, a1.pi, res.point1, a1.way));
            ns2 = createArc(arc2PC2SVG({ x: a2.cx, y: a2.cy }, a2.r, res.point2, a2.pf, a2.way));
        }
    ...
    if (res.r === Cut.NoIntersect) {
        ...
        let ns = createArc(
            arc2PC2SVG({ x: origin.pf.x, y: origin.pf.y }, Math.abs(join_params.offset), a1.pf, a2.pi, join_params.connection_arcs_ccw),
        );
        return { res: [a1, ns], left: a2 };
    }
    let point;
    if (res.r === Cut.OneIntersect || res.r === Cut.TangentIntersect) {
        //Supongo que tengo que cortar como si hubiera solo 1 intersect...
        point = res.point;
...
function createRawOffsetPath(path, offset, options) {
    ...
    //En realidad, si el path estaba cerrado y queremos hacer el offset, cualquier cosa que no tenga tres segmentos,
    //  o segmento y arco o dos arcos como mínimo sería error...
    ...
    if (pathIsClosed(path)) {
        //al cerrar , además de el último tramo, puede modificarse el comienzo del primero
        let joint = joinOffsetShapes(shape, rawOffsetSegs[0], join_params);
        result = result.concat(joint.res);
        if (!fuzzy_eq_point(result[0].pi, joint.left.pi)) {
            let old = result[0];
            if (joint.left.type === "segment") result[0] = createSegment({ x0: joint.left.pi.x, y0: joint.left.pi.y, x1: old.pf.x, y1: old.pf.y });
            else result[0] = createArc(arc2PC2SVG({ x: old.cx, y: old.cy }, old.r, joint.left.pi, old.pf, old.way));
        }
    }
...
function slicesFromRawOffset(path, rawOffsetPath, offset, options) {
    ...
    if (selfIntersects.basic.length === 0) {
        // no self intersects, test point on polyline is valid
        if (!sliceIsValid(rawOffsetPath.elements[0], path, offset)) return result;
...
export const defaultOffsetOptions = {
    handle_self_intersects: false,
    pos_equal_eps: geometryPrecision,
    slice_join_eps: geometryPrecision,
    offset_dist_eps: geometryPrecision,
};
...
export function parallelOffset(path, offset, options = defaultOffsetOptions) {
    ...
    if (rawOffsetPath.elements.length === 0) return { error: true, paths: [], text: "Error al calcular los offsets de los paths" };
