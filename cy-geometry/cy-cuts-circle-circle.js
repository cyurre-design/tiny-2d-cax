...
export function circle_circle_intr( c1, c2, pos_equal_eps= geometryPrecision)
{
    ...
    if(fuzzy_eq_zero(d, pos_equal_eps)) {           //mismo centro
        if(fuzzy_eq(c1.r, c2.r, pos_equal_eps) ){   //mismo radio
    ...
    if (!fuzzy_lt(d, c1.r + c2.r, pos_equal_eps)    //círculos disjuntos, d >= r1+r2
...
export function arc_arc_intr(a1, a2, pos_equal_eps=geometryPrecision){
    ...
    switch(intr_result.r) {
        ...
        case Cut.TangentIntersect:{
            ...
            if      (point_lies_on_arc1(a2.pi)) {  intr_result.point = a2.pi; }
            else if (point_lies_on_arc1(a2.pf)) {  intr_result.point = a2.pf; }
            else if (point_lies_on_arc2(a1.pi)) {  intr_result.point = a1.pi; }
        ...
        case Cut.TwoIntersects: {
            ...
            else if(pt1_in_sweep) { return {r:Cut.OneIntersect, point: pt1}}
            else if(pt2_in_sweep) { return {r:Cut.OneIntersect, point: pt2}}
...
export function circle_arc_intr(c, a2, pos_equal_eps=geometryPrecision){
    ...
    switch(intr_result.r) {
        case Cut.NoIntersect:
                return intr_result;
        case Cut.TangentIntersect:{ //caso de 1 solo punto de corte, circulos tangentes, ver si está en el arco
            // first check if end points lie on arcs and substitute with end point if so to be
            // consistent with stickiness to end points done in other cases (e.g., line-arc intersect)
            //Si el punto inicial o final están en la cirunferencia, devolvemos el original
            if      (point_lies_on_circle(a2.pi)) {  intr_result.point = a2.pi; }
            else if (point_lies_on_circle(a2.pf)) {  intr_result.point = a2.pf; }
