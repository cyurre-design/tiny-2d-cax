...
export function linkPaths(allPaths, tol = 0.1) {
    ...
    function blocksToPaths(allBlocks, tol) {
        ...
        let linked = [],
            head = { x: Infinity, y: Infinity },
            tail = { x: Infinity, y: Infinity }
        ...
        for (let ix = 0; ix < blocks.length; ix++) {
            b = blocks[ix]
            if (fuzzy_eq_point(tail, b.pi, tol)) {
                //por la "derecha"
                linked.push(b)
                tail = b.pf
            } else if (fuzzy_eq_point(tail, b.pf, tol)) {
                //por la "derecha" invertido
                linked.push(blockReverse(b)) //Tiene que ser un reverse no destructivo porque si no no furrula
                tail = b.pi
            } else if (fuzzy_eq_point(head, b.pf, tol)) {
                //por la "izquierda"
                linked.unshift(b)
                head = b.pi
            } else if (fuzzy_eq_point(head, b.pi, tol)) {
                //por la izquierda invirtiendo
                linked.unshift(blockReverse(b))
                head = b.pf
            } else {
                //Uno que no empalma...el primero será uno vacío
    ...
    function stitch(paths, tol) {
        ...
        while (1) {
            ...
            for (let ix = 0; ix < paths.length; ix++) {
                ...
                head = linked[0].pi
                tail = linked[linked.length - 1].pf
            }
    ...
    //test2
    if (slices.find((p) => p.elements.find((b) => b.type === 'path'))) console.log('error')
