...
import { geometryPrecision, fuzzy_eq_point, blockMidpoint } from './cy-geometry-library.js'
...
export function pathBoolean( path1, path2, op,  options = {pos_equal_eps: geometryPrecision}){
    ...
    if(pathOrientation(path1) !== pathOrientation(path2))
        path2 = pathReverse(path2);
    ...
    if(intrs.basic.length === 0 && intrs.overlapping.length === 0){
        ...
        switch(operation){
            ...
            case 'NOT'  :
                if(is_path1_in_path2) {         // everything is subtracted (nothing left)
                    return ({posPaths: [] , negPaths:[], r:'Pline1InsidePline2'});
                } else if(is_path2_in_path1) {  // negative space island created inside pline1
