...
export class BSegment extends Segment {
    ...
    closestPoint(point, eps = geometryPrecision) {
        ...
        if (l > this.d) return { x: this.pi.x + this.ux * l, y: this.pi.y + this.uy * l };
    }
...
export class BPath extends Path {
    ...
    area() {
        ...
        this.elements.forEach((shape) => {
            if (shape) double_total_area = double_total_area + shape.pi.x * shape.pf.y - shape.pi.y * shape.pf.x;
    ...
    orientation() {
        if (!this.isClosed) return "open";
        return this.area() < 0 ? "clock" : "antiClock";
    }
    //Esta no se usará casi nunca, de hecho se usa solo en tests, así que no optimizo nada
    changeStart(n) {
        //No clona, solo reordena, los quita de la cabeza y los empalma por la cola (se puede hacer de otras maneras)
        if (n === 0 || n >= this.elements.length) return;
        for (let i = 0; i < n; i++) this.elements.push(this.elements.shift()); //YURRE: igual no es la más óptima, pero bueno...es la más clara
    }
    ...
    static createPathFromVertexes(vertexes, closed = false) {
        ...
        if (Array.isArray(dxfVertexArray)) {
            if (!Array.isArray(dxfVertexArray[0])) {
                //Me pasan un solo vertes
            ...
            if (dxfVertexArray.length >= 2) {
                ...
                if (closed) dxfVertexArray.push(dxfVertexArray[0]); //no pongo bulge a 0 porque no se debería usar....
                ...
                for (let i = 0; i < nSegments; i++) {
                    ...
                    if (vertex[2] === 0) shapes.push(new BSegment(vertex[0], vertex[1], next[0], next[1]));
                    else {
    ...
    static fromJSON(str) {
        //Esta debe estar en Path , de hecho la hago para path y luego llamo al conversor general
        let npaths = JSON.parse(str);
        npaths = Array.isArray(npaths) ? npaths : [npaths];
        let nBPaths = npaths.map((p) => {
            let elements = [];
            p.args.forEach((el) => {
                //YURRE, solo trato de momento los básicos
                if (el.type === "segment") {
                    ...
                    elements.push(
                        new Arc(
                            ...
                            el.args.y,
                            el.args.r,
                            { x: el.args.pi.x, y: el.args.pi.y },
                            { x: el.args.pf.x, y: el.args.pf.y },
                            el.args.way,
                        ),
                    );
                }
    ...
    arcsToApproxLines(error_cordal, eps) {
        ...
        this.elements.forEach((shape) => {
            if (shape instanceof BSegment) elements.push(shape.clone());
            if (shape instanceof BArc) {
                if (fuzzy_lt(shape.r, error_cordal, eps)) elements.push(new BSegment(shape.pi.x, shape.pi.y, shape.pf.x, shape.pf.y));
                else {
    ...
    removeRedundant(options = { pos_equal_eps: geometryPrecision, invert_area: false }) {
        ...
        function notCollapsable(el) {
            ...
            if (elArc !== onArc || !fuzzy_eq_point(el.pi, actual.pf, eps)) return true; //no colapsa
            if (!isArc(actual)) {
                //segmento
        ...
        if (this.isClosed && elements.length > 0) {
            if (notCollapsable(elements[0]) === false) {
...
export class ComplexPath {
    constructor(antiClockPaths, clockPaths) {
        ...
        if (antiClockPaths.some((p) => !(p instanceof BPath))) {
            console.log("entrada errónea");
            return;
        }
        if (clockPaths.some((p) => !(p instanceof BPath))) {
            console.log("entrada errónea");
            return;
        }
        this.cajeras = antiClockPaths;
        this.islas = clockPaths;
        this.cajeras.forEach((p) => {
            //Habría que dar warning o error??
            if (p.orientation() !== "antiClock") p.reverse();
        });
        this.islas.forEach((p) => {
            if (p.orientation() !== "clock") p.reverse();
        });
    }
    clone() {
        return new ComplexPath(
            this.cajeras.map(
                (antiClock) => antiClock.clone(),
                this.islas.map((clock) => clock.clone()),
            ),
        );
    }
}
...
export function winding_number(path, point) {
    ...
    let winding = path.elements.reduce(
        (winding, el) => winding + (el instanceof BSegment ? process_line_winding(el, point) : process_arc_winding(el, point)),
        0,
    );
    return winding;
}
export function path2BPath(path) {
    let bEls = path.elements.map((el) => {
        if (el.type === "segment") return new BSegment(el.pi.x, el.pi.y, el.pf.x, el.pf.y);
        else if (el.type === "arc") return new BArc(el.x, el.y, el.r, el.pi, el.pf, el.pathway === 1 ? "antiClock" : "clock");
        else {
...
export function bpath2Path(path) {
    let els = path.elements.map((el) => {
        if (el instanceof BSegment) return new Segment(el.pi.x, el.pi.y, el.pf.x, el.pf.y);
        else if (el instanceof BArc) return new Arc(el.x, el.y, el.r, el.pi, el.pf, el.pathway === 1 ? "antiClock" : "clock");
        else {
            console.log("elemento no contemplado"); //YURRE: TODO convertir chaflanes y redondeos
        }
    });
