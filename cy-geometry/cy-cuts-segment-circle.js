import { geometryPrecision, fuzzy_eq, fuzzy_eq_point, fuzzy_eq_zero, sqDistancePointToPoint, pointWithinArcSweep } from './cy-geometry-library.js'
...
export function segment_arc_intr(segment, arc, eps = geometryPrecision) {
    ...
    if (fuzzy_eq_zero(dx, eps)) {
        ...
        // caso normal, YURRE: creo que tendríamos los datos en nuestra clase, pero mantengo riginal
        // (y - y1) = m(x - x1)     =>    y - y1 = mx - mx1 =>  mx - y + y1 - mx1 = 0
        // A = -m   ,   B = 1.0,    C = -y1 + m*x1, m = (y1 - y0) / (x1 - x0)
        let m = dy / dx
        a = m
        ;((b = -1.0), (c = pf_shifted.y - m * pf_shifted.x))
    }
    ...
    const point_lies_on_arc_and_segment = (pt) => {
