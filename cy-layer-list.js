...
`;
    ...
    createNewList() {
        const list = this.layers.reduce(
            (out, ly) => out + templateSingleLayer(ly.name, ly.lyId, ly.erasable),
            '<div id="full-list" class="column"></div>',
        );
        return list;
    }
    ...
    handleLayers = (e) => {
        ...
        this.dispatchEvent(
            new CustomEvent("layer-handle", { bubbles: true, composed: true, detail: { layerId: idn, action: "visibility", value: s } }),
        );
    };
    connectedCallback() {
        ...
        this.list.addEventListener(`click`, (evt) => {
            ...
            if (command === "edit") {
                ...
                this.dispatchEvent(
                    new CustomEvent("layer-handle", { bubbles: true, composed: true, detail: { layerId: this.editedLayer.lyId, action: "delete" } }),
                );
            } else if (command === "id") {
        ...
        this.dom.querySelector("#edit-style").addEventListener("click", (e) => {
            if (e.target.id === "style-enter") {
                ...
                ((this.editedLayer.name = this.dialog.querySelector("#layer-name").value), this.dialog.close());
        ...
        this.dom
            .querySelector("#layer-add")
            .addEventListener("click", (e) =>
                this.dispatchEvent(
                    new CustomEvent("layer-handle", { bubbles: true, composed: true, detail: { layer: undefined, action: "create" } }),
                ),
            );
    }
    ...
    addLayer(strLayer) {
        ...
        if (layer.erasable) this.setActiveLayerClass(layer.lyId);
    }
    /**
     *
     * @param {@todo} name
