...
export default class CyInputDataMeasure extends HTMLElement {
    ...
    set subType(type) {
        ...
        ["measure-block", "measure-p2p", "measure-rotate"].forEach((el) => (this.dom.querySelector("#" + el).style.display = "none"));
    ...
    createTemplate() {
        ...
        h += `<div id="${t}" style="display:none;">
			<div>${TX0Y0(t)}${TX1Y1(t)}</div>
        	<div class="row">MEAS. LENGTH<input id='data-${t}-len' value="0" class="_33"/></div>
			<div class="row">SET LENGTH<input id='data-${t}-truelen' class="_33" value="0" /></div>
		</div>`;
        t = "measure-rotate";
        h += `<div id="${t}" style="display:none;">`;
        h += `${TSA(t)}</div>`;
